...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { userImage, referenceImage, userPrompt, highRes = true } = await request.json()

    if (!userImage || typeof userImage !== 'string') {
      return NextResponse.json({ status: 'error', message: 'userImage is required' }, { status: 400 })
    }

    if (!referenceImage || typeof referenceImage !== 'string') {
      return NextResponse.json({ status: 'error', message: 'referenceImage is required' }, { status: 400 })
    }
    ...
    const aiRes = await fetch(`${backend}/api/identity-swap`, {
      method: 'POST',
      headers: {
    ...
    if (!aiRes.ok || !aiData?.success || !aiData?.image) {
      await prisma.user.update({ where: { id: userId }, data: { credits: { increment: 1 } } })
      return NextResponse.json({ status: 'error', message: aiData?.message || 'Identity swap failed' }, { status: 502 })
    }

    let finalImage: string = aiData.image
    ...
    if (didReserveCredit && userId) {
      try {
        ...
        console.error('[v0] Image identity-swap refund error:', refundError)
      }
    }

    console.error('[v0] Image identity-swap API error:', error)
    const message =
